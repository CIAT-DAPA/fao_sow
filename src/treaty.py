...
class Treaty(object):
    ...
    def __init__(self, inputs_folder, input_file, input_sheet, workspace, outputs= 'treaty', encoding="utf-8"):
    ...
    def merge_plant_treaty(self, plant_treaty, plant_treaty_fields, plant_treaty_key, crop_key, step="01",force=False):
        ...
        if force or not os.path.exists(final_file):
            ...
            print("\tMergin columns with similar data")
            df_merged = self.update_columns_data(df_merged,"Use - detailed")
            df_merged = self.update_columns_data(df_merged,"Use - primary")
            df_merged = self.update_columns_data(df_merged,"Included in MLS Annex 1")
    ...
    # (bool) force: Set if the process have to for the execution of all files even if the were processed before.
    #               By default it is False
    def merge_countries(self, origin, recipient, countries_file, countries_sheet, countries_fields, countries_key, step="02",force=False):
        final_path = os.path.join(self.getworkspace(),step)
        ...
        if force or not os.path.exists(final_file):
            ...
            print("\tProcessing countries=", countries_path,"data=",os.path.join(self.getworkspacestep("01"),self.outputs + ".csv"))
            countries_xls = pd.ExcelFile(countries_path)
            df_countries = countries_xls.parse(countries_sheet)
            c_fields_list = countries_fields
            #c_fields_list.append(countries_key)
            ...
            df_merged = pd.merge(df_merged,df_countries,how="left",left_on=[recipient],right_on=[countries_key])
            print("\tNumber rows: treaty=",df.shape[0],"countries=",df_countries.shape[0],"merged=",df_merged.shape[0])
    ...
    # (string) origin: Field name for key of country from origin
    # (string) recipient: Field name for key of country from recipient
    # (string) treaty_year: Field name for year of transaction
    ...
    # (bool) force: Set if the process have to for the execution of all files even if the were processed before.
    #               By default it is False
    def merge_income(self, origin, recipient, treaty_year, income_file, income_sheet, income_key, income_years, step="03",force=False):
        final_path = os.path.join(self.getworkspace(),step)
        mf.create_review_folders(final_path, sm=False)
        # It checks if files should be force to process again or if the path exist
        final_file = os.path.join(final_path,"OK",self.outputs + ".csv")
        if force or not os.path.exists(final_file):
            ...
            print("\tProcessing income=", income_path,"data=",os.path.join(self.getworkspacestep("02"),self.outputs + ".csv"))
            ...
            print("\tMerging origin")
            df_merged = pd.merge(df,df_income,how="left",left_on=[origin,treaty_year],right_on=[income_key,"variable"])
            print("\tNumber rows: treaty=",df.shape[0],"income=",df_income.shape[0],"merged=",df_merged.shape[0])

            print("\tMerging recipient")
            df_merged = pd.merge(df_merged,df_income,how="left",left_on=[recipient,treaty_year],right_on=[income_key,"variable"])
            print("\tNumber rows: treaty=",df.shape[0],"income=",df_income.shape[0],"merged=",df_merged.shape[0])

            df_merged = df_merged.drop(["variable_x","variable_y",income_key+"_x",income_key+"_y"], axis=1)
    ...
    # (string) treaty_key_germplasm: Field name for key of country from origin
    ...
    # (bool) force: Set if the process have to for the execution of all files even if the were processed before.
    #               By default it is False
    def merge_germplasm(self, treaty_key_germplasm, germplasm_file, germplasm_sheet, germplasm_key, step="04",force=False):
        final_path = os.path.join(self.getworkspace(),step)
        mf.create_review_folders(final_path, sm=False)
        # It checks if files should be force to process again or if the path exist
        final_file = os.path.join(final_path,"OK",self.outputs + ".csv")
        if force or not os.path.exists(final_file):
            ...
            print("\tProcessing germplasm=", germ_path,"data=",os.path.join(self.getworkspacestep("03"),self.outputs + ".csv"))
            ...
            df_merged = pd.merge(df,df_germ,how="left",left_on=[treaty_key_germplasm],right_on=[germplasm_key])
            print("\tNumber rows: treaty=",df.shape[0],"income=",df_germ.shape[0],"merged=",df_merged.shape[0])
